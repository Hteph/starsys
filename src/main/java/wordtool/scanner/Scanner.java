...
public class Scanner {
    ...
    private void processScanConfig(
        final ScanConfig sc,
        final Map<String, Set<String>> wordMap,
        final Set<String> words) {
        Set<String> wordSet =
            wordMap.computeIfAbsent(sc.getExprAndFlags() == null ? "" : sc.getExprAndFlags(), k -> new TreeSet<>());
    ...
    private Map<String, Set<String>> generateStartMap(final FrequencyStats fs, final FrequencyStats fsLonger) {
        ...
        for (final Entry<String, FrequencyStats.FrequencyPositionStats> e : fs.getCountMap().entrySet()) {
            if (e.getValue().getStartCount() > 0) {
                for (final Entry<String, FrequencyStats.FrequencyPositionStats> ce : CollectionsUtil.getByPrefix(
                    fsLonger.getCountMap(),
                    e.getKey()
                ).entrySet()) {
                    if (ce.getValue().getStartCount() > 0) {
                        ...
                        for (final Entry<String, FrequencyStats.FrequencyPositionStats> subCe :
                            CollectionsUtil.getByPrefix(
                                fsLonger.getCountMap(),
                                subKey
                            ).entrySet()) {
