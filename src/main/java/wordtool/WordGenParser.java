...
import wordtool.ast.ListSyllable;
import wordtool.ast.Syllable;
import wordtool.ast.expression.CharacterMatchExpression;
import wordtool.ast.expression.Expression;
import wordtool.ast.expression.parser.*;
...
public class WordGenParser {
	...
	private Syllable parseLine(final String line) {
    	...
    	Syllable.SyllablePosition sylPos ;
    	
    	int curIndex = 0;
    	//determine position
    	if (line.startsWith("+")) {
    		sylPos = Syllable.SyllablePosition.END;
    		curIndex = 1;
    	} else if (line.startsWith("-")) {
    		sylPos = Syllable.SyllablePosition.START;
    		curIndex = 1;
    	} else if (line.startsWith("=")) {
    		sylPos = Syllable.SyllablePosition.ANY;
    		curIndex = 1;
    	} else {
